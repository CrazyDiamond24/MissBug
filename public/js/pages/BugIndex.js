...
export default {
  template: `
    ...
    <router-link class="add-bug" to="/bug/edit">Add New Bug</router-link> 
        <bug-list v-if="bugs" :bugs="bugs" @removeBug="removeBug">
		{{bugsToDisplay}}
		</bug-list>
        <div class="btns-paging">
        <button class="paging" @click="getPage(-1)">Prev</button>
        <button class="paging" @click="getPage(1)">Next</button>
        </div>
